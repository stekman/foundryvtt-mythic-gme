...
import {DEFAULT_PANEL_KEYS, normalizePanelKeys, PANEL_DEFINITIONS} from "./app/panel-registry";
...
export default class MGMEMacroAPI {
  ...
  static mgmeRenderPanel(panelKeys) {
    ...
    win.render({
      ...
      position: {
        ...
        left: (canvas.app.screen.width - activePanel.width - 400),
        top: canvas.app.screen.height - 455,
  ...
  static async mgmeMigratePanelKeys() {
    ...
    const panelKeys = legacyPanelKey === 'nopanel' ? [] : (migratedKeys.length ? migratedKeys : DEFAULT_PANEL_KEYS);
